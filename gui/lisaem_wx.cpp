...
static void iw_check_finish_job(void);


extern void turn_skins_on(void);
extern void turn_skins_off(void);

static void powerbutton(void);
static void setvideomode(int mode);
static void save_global_prefs(void);
...
static char *get_welcome_fortune(void);
...
static void set_hidpi_scale(void);
...
static void buildscreenymap(void)
{
#define MAXHY 588
#define MAXY  500
...
static void buildscreenymap_raw(void)
{
 wxCoord y;
ALERT_LOG(0,"building...");
 for (y=0; y<364; y++)
...
static void buildscreenymap_3A(void)
{
 wxCoord y;
 ALERT_LOG(0,"building...");
 for (y=0; y<431; y++)
...
static void buildscreenymap_2Y(void)
{
 wxCoord y;
 ALERT_LOG(0,"building...");
 for (y=0; y<364; y++)   yoffset[y]=y*90;
...
static void buildscreenymap_3Y(void)
{
 wxCoord y;
 ALERT_LOG(0,"building...");
 for (y=0; y<364; y++)   yoffset[y]=y*90;
...
static void banner(void)    { puts(getbanner());}
...
static void handle_powerbutton(void)
{
      ALERT_LOG(0,"======== ENTRY ================");
...
//      : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(ISKINSIZE), wxDEFAULT_FRAME_STYLE)
{
   ...
   if   (x>screensizex || y>screensizey)  // make sure we don't get too big
    ...
    resdir = wxFileName(stdp.GetExecutablePath()).GetPath() + osslash;
    skindir  =  resdir   + "skins" + osslash + skinname + osslash;     // /usr/local/share/LisaEm/skins/default
...
static void setvideomode(int mode) {if (!!my_lisawin) my_lisawin->SetVideoMode(mode);}

//----- ImageWriter interfaces to old C code. -----//
#include <printer/imagewriter-wx.h>
...
extern uint8 evenparity(uint8 data)            // from http://graphics.stanford.edu/~seander/bithacks.html#ParityParallel
{
  uint32 v=data;
